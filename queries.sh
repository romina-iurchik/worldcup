...
echo "$($PSQL " SELECT DISTINCT(teams.name) FROM teams INNER JOIN games ON (teams.team_id = games.winner_id OR teams.team_id = opponent_id) WHERE round
= 'Eighth-Final' AND year = 2014;")"
...
echo "$($PSQL "SELECT teams.name FROM teams INNER JOIN games ON teams.team_id = games.winner_id WHERE teams.name LIKE 'Co%';")"
